...
namespace RTSEngine.NPC.ResourceExtension
{
    public class NPCTerritoryManager : NPCComponentBase, INPCTerritoryManager
    {
        ...
        private void UpdateCurrentTerritory(float value)
        {
            ...
            LogEvent($"Current territory ratio updated: {currentTerritoryRatio}");
        }
        #endregion
        ...
        public void OnExpandRequest()
        {
            if (!expandOnDemand)
            {
                LogEvent($"External Expansion demand rejected!");
                return;
            }
        ...
        {
            string nextCenterCode = centerMonitor.RandomCode;
            LogEvent($"Sending Expansion demand with building {nextCenterCode}");
            npcBuildingCreator.OnCreateBuildingRequest(
                buildingCode: nextCenterCode,
        ...
        #region Logging
        [System.Serializable]
        private struct TerritoryLogs
        {
            public float currTerritoryRatio;
        }
        [SerializeField, ReadOnly]
        private TerritoryLogs territoryLogs = new TerritoryLogs();

        protected override void UpdateActiveLogs()
        {
            territoryLogs = new TerritoryLogs
            {
                currTerritoryRatio = currentTerritoryRatio,
            };
        }
        #endregion
    }
