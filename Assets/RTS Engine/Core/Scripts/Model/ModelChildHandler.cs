...
namespace RTSEngine.Model
{
    ...
    {
        #region Attributes
        private IDictionary<int, Material> materialsDic = null;
        ...
        {
            int index = -1;
            materialsDic = this.current
                .materials
                .ToDictionary(material => { index++; return index; }, material => material);
        ...
        {
            if (!materialsDic.ContainsKey(materialID))
                return false;

            materialsDic[materialID] = material;

            if (!current.IsValid())
                return true;

            Material[] tempArray = current.materials;
            tempArray[materialID] = material;
            current.materials = tempArray;
            return true;
        }
        #endregion
        ...
        protected override void OnShow()
        {
            foreach (KeyValuePair<int, Material> kvp in materialsDic)
                current.materials[kvp.Key] = kvp.Value;
